...
export default function WhyUs() {
  return (
    <section id="about" className="bg-[#0F2742] py-24 text-[#F4F8FF] md:py-32" aria-label="Why us">
      <div className="lux-container grid gap-16 md:grid-cols-12 md:gap-12">
        <div className="md:col-span-5 md:col-start-2">
          <p className="section-label text-[#DFEAF7]/70">Por que nosotros</p>
          <h2 className="mt-5 font-[family-name:var(--font-display)] text-5xl leading-[0.92] tracking-tight sm:text-6xl md:text-7xl">
            Menos ruido,
            <br />
            mas <span className="italic text-[#2F78C4]">claridad</span>
          </h2>
          ...
          {points.map((p, i) => (
            <article
              ...
              className={`group flex gap-5 py-7 transition-[padding] duration-500 ease-out ${
                i !== points.length - 1 ? "border-b border-[#DFEAF7]/20" : ""
              }`}
            >
              <div className="mt-1 shrink-0">
                <p.Icon size={22} className="text-[#DFEAF7]/80 transition-colors duration-500 group-hover:text-[#2F78C4]" strokeWidth={1.5} />
              </div>
              <div>
                <h3 className="mb-1 font-[family-name:var(--font-display)] text-2xl leading-tight text-[#F4F8FF] transition-colors duration-500 group-hover:text-[#2F78C4]">
                  {p.title}
                </h3>
                <p className="text-base leading-relaxed text-[#DFEAF7]/75">{p.desc}</p>
              </div>
            </article>
          ))}
        </div>
